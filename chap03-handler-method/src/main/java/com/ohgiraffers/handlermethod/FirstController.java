...
public class FirstController {
    ...
    /* @RequestParam으로 요청 파라미터 전달받기
    * 요청 파라미터를 매핑하여 호출 시 값을 넣어주는 어노테이션으로 매개변수 앞에 작성한다.
    * form의 name 속성값과 매개변수의 이름이 다른경우 @RequestParam("name")을 설정하면 된다.
    *
    * 값이 넘어오지 않는 경우 dafaultValue를 이용해 기본값을 설정해 줄 수 있다.*/
    @PostMapping("modify")
    public String modifyMenuPrice(Model model, @RequestParam String modifyName, @RequestParam(defaultValue = "0") int modifyPrice) {
...
//        System.out.println(menuDTO);
    ...
    /* HttpSession을 매개변수로 선언하면 핸들러 메소드 홀출 시 세션 객체를 넣어서 호출한다. */
    @PostMapping("login1")
    ...
    public String  logoutTest1(HttpSession session) {
        ...
        session.invalidate();
        //@SessionAttributes("id")로 로그인된 경우 로그아웃 되지 않음
    ...
    @GetMapping("logout2")
    public String  logoutTest1(SessionStatus sessionStatus) {
